...
/**
 * Karzintell initial MySQL/MariaDB schema.
 *
 * This migration is intentionally written as raw MySQL SQL so it is portable
 * between MySQL 8.x and MariaDB 10.5+ (the typical cPanel stack). It mirrors
 * database/20260825000000_initial_store_mysql.sql exactly.
 *
 * The TypeORM "migrations" table is used by the built-in migration runner to
 * make repeated `npm run db:migrate` calls idempotent.
 */
export class InitialStoreMySql1750000000000 implements MigrationInterface {
  name = 'InitialStoreMySql1750000000000';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(tableSql);
    await queryRunner.query('SET FOREIGN_KEY_CHECKS = 1;');

    // Standalone indexes are kept in the canonical schema. If a deployment had
    // already imported the same schema manually, MySQL raises ER_DUP_KEYNAME;
    // that is harmless and is ignored so the migration stays idempotent.
    for (const sql of [
