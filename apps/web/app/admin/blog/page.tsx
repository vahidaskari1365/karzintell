            ...
            <tbody>
              {items.map((p) => (
                <tr key={p.id} className={tableCls.row}>
                  ...
                  <td className={tableCls.td}><code className="text-2xs text-slate-600 dark:text-slate-200" dir="ltr">{p.slug}</code></td>
                  <td className={tableCls.td}><Pill status={p.status === 'published' ? 'active' : 'pending'} label={p.status === 'published' ? 'منتشرشده' : 'پیش‌نویس'} /></td>
                  <td className={tableCls.td}><span className="text-2xs text-slate-600 dark:text-slate-200">{p.publishedAt ? faDate(p.publishedAt) : '—'}</span></td>
                  <td className={`${tableCls.td} text-left`}>
                    <div className="flex justify-end gap-1">
                      ...
                      <button onClick={() => setDeleting(p)} className="rounded-lg p-2 text-slate-600 dark:text-slate-200 hover:bg-rose-50 hover:text-rose-600"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
              {items.length === 0 && <tr><td colSpan={5} className="p-8 text-center text-xs text-slate-600 dark:text-slate-200">هنوز چیزی ثبت نشده است</td></tr>}
            </tbody>
          </table>
        </div>
      )}
