                ...
                <tr key={c.id} className={tableCls.row}>
                  <td className={tableCls.td}>
                    <span style={{ paddingInlineStart: c.depth * 22 }} className="flex items-center gap-1.5 font-medium">
                      {c.depth > 0 && <span className="text-slate-700 dark:text-slate-200">└</span>}
                      {c.name}
                    </span>
                  </td>
                  <td className={tableCls.td}><code className="text-xs text-slate-600 dark:text-slate-200" dir="ltr">{c.slug}</code></td>
                  ...
                  <td className={`${tableCls.td} text-left`}>
                    <div className="flex justify-end gap-1">
                      ...
                      <button onClick={() => setDeleting(c)} className="rounded-lg p-2 text-slate-600 dark:text-slate-200 hover:bg-rose-50 hover:text-rose-600">
  ...
  return (
    <Dialog open onClose={onClose} title={`ویژگی‌های «${category.name}»`}>
      {isLoading || !all ? (
        ...
        <div className="space-y-4">
          <p className="text-xs text-slate-600 dark:text-slate-200">ویژگی‌های انتخاب‌شده در فیلتر فروشگاه می‌آیند؛ «سازنده تنوع» یعنی از آن برای ساخت تنوع محصول (مثل رنگ/حافظه) استفاده می‌شود.</p>
          <ul className="max-h-80 space-y-2 overflow-y-auto">
            {all.map((a) => {
              const cur = state[a.id];
              return (
                <li key={a.id} className="flex items-center gap-3 rounded-xl border border-slate-200 dark:border-slate-800 p-2.5">
                  <input
                    type="checkbox"
                    checked={!!cur}
                    onChange={(e) => {
                  ...
                  />
                  <span className="flex-1 text-sm font-medium text-slate-700 dark:text-slate-200">{a.name} <code className="text-2xs text-slate-600 dark:text-slate-200" dir="ltr">({a.code})</code></span>
                  {cur && (
                    <>
                      <label className="flex items-center gap-1 text-2xs text-slate-600 dark:text-slate-200">
