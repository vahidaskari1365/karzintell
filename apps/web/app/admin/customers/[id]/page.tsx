  ...
  return (
    <div>
      <div className="mb-5 flex items-center gap-3">
        <Link href="/admin/customers" className="rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900/40 p-2 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-slate-100">
          <ArrowRight className="h-4.5 w-4.5" />
        </Link>
        <div>
          <h1 className="text-xl font-black text-slate-900 dark:text-slate-100">{user.fullName}</h1>
          <p className="mt-0.5 text-xs text-slate-600 dark:text-slate-400" dir="ltr">{user.phone}{user.email ? ` · ${user.email}` : ''}</p>
        </div>
        ...
        <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            <Card key={k.label} className="p-4">
              <p className="mb-1 flex items-center gap-1.5 text-2xs text-slate-600 dark:text-slate-400">{k.icon}{k.label}</p>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="p-0 lg:col-span-2">
          <p className="border-b border-slate-200 dark:border-slate-800 px-5 py-4 text-sm font-bold text-slate-800 dark:text-slate-200">آخرین سفارش‌ها</p>
            ...
            <table className={tableCls.table}>
              <tbody>
                ...
                {(d.orders || d.recentOrders || []).length === 0 && (
                  <tr><td className="p-6 text-center text-xs text-slate-600 dark:text-slate-400">سفارشی ندارد</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <Card className="p-5">
          ...
          {(d.addresses || []).length === 0 ? (
            <p className="text-xs text-slate-600 dark:text-slate-400">آدرسی ثبت نشده</p>
          ) : (
            <ul className="space-y-2 text-xs leading-6 text-slate-600 dark:text-slate-400">
              {d.addresses.map((a: any) => (
                <li key={a.id} className="rounded-xl border border-slate-200 dark:border-slate-800 p-3">
                  ...
                  <p className="text-slate-600 dark:text-slate-400" dir="ltr">{a.receiverPhone}</p>
                </li>
              ))}
            </ul>
          )}
          <p className="mt-4 border-t border-slate-200 dark:border-slate-800 pt-3 text-2xs text-slate-600 dark:text-slate-400">عضویت: {faDate(user.createdAt)}</p>
          {user.lastLoginAt && <p className="text-2xs text-slate-600 dark:text-slate-400">آخرین ورود: {faDateTime(user.lastLoginAt)}</p>}
        </Card>
      </div>
    </div>
  );
