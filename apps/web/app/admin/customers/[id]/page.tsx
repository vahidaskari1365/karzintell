  ...
  return (
    <div>
      <div className="mb-5 flex items-center gap-3">
        <Link href="/admin/customers" className="rounded-xl border border-slate-700 bg-slate-900/40 p-2 text-slate-400 hover:text-slate-100">
          <ArrowRight className="h-4.5 w-4.5" />
        </Link>
        <div>
          <h1 className="text-xl font-black text-slate-100">{user.fullName}</h1>
        ...
        <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            <Card key={k.label} className="p-4">
              ...
              <p className="text-base font-black text-slate-100">{k.value}</p>
            </Card>
          ))}
        </div>
      )}
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="p-0 lg:col-span-2">
          <p className="border-b border-slate-800 px-5 py-4 text-sm font-bold text-slate-200">آخرین سفارش‌ها</p>
        ...
        <Card className="p-5">
          <p className="mb-3 flex items-center gap-1.5 text-sm font-bold text-slate-200"><MapPin className="h-4 w-4" /> آدرس‌ها</p>
          {(d.addresses || []).length === 0 ? (
            ...
            <ul className="space-y-2 text-xs leading-6 text-slate-400">
              {d.addresses.map((a: any) => (
                <li key={a.id} className="rounded-xl border border-slate-800 p-3">
                  <p className="font-bold text-slate-300">{a.title}</p>
          ...
          <p className="mt-4 border-t border-slate-800 pt-3 text-2xs text-slate-400">عضویت: {faDate(user.createdAt)}</p>
