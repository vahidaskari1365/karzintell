          ...
          <div className="mt-3 grid gap-2 md:grid-cols-2">
            {alerts.map((alert) => (
              <div key={alert.id} className="flex items-center justify-between rounded-xl bg-white dark:bg-slate-900/40/70 px-3 py-2 text-xs">
                <div><b>{alert.productName}</b><span className="ms-2 text-slate-600 dark:text-slate-200" dir="ltr">{alert.sku}</span></div>
                ...
                {items.map((r) => (
                  <tr key={`${r.variantId}-${r.warehouseId}`} className={tableCls.row}>
                    <td className={tableCls.td}>
                      ...
                      <p className="text-2xs text-slate-600 dark:text-slate-200" dir="ltr">{r.sku}{r.variantTitle ? ` — ${r.variantTitle}` : ''}</p>
                    </td>
                    ...
                    <td className={`${tableCls.td} text-left`}>
                      <div className="flex justify-end gap-1">
                        ...
                        <button onClick={() => setHistoryFor(r)} className="rounded-lg p-2 text-slate-600 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800/40 hover:text-slate-700 dark:text-slate-200" title="تاریخچه گردش">
  ...
  return (
    <Dialog open onClose={onClose} title={`اصلاح موجودی — ${row.productName}`}>
      <div className="space-y-4">
        <p className="text-xs text-slate-600 dark:text-slate-200" dir="ltr">{row.sku}</p>
            ...
            {(data?.data || []).map((m) => (
              <li key={m.id} className="rounded-xl border border-slate-200 dark:border-slate-800 p-3 text-xs">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-slate-600 dark:text-slate-200">{faDateTime(m.createdAt)}</span>
                </div>
                <p className="mt-1.5 text-slate-600 dark:text-slate-200">
                  {faNumber(m.quantity)} عدد — موجودی: {faNumber(m.qtyBefore)} ← {faNumber(m.qtyAfter)}
                </p>
                {m.note && <p className="mt-1 text-slate-600 dark:text-slate-200">{m.note}</p>}
              </li>
            ))}
            {(data?.data || []).length === 0 && <p className="p-6 text-center text-xs text-slate-600 dark:text-slate-200">گردشی ثبت نشده</p>}
          </ul>
