  ...
  const nav = (
    <nav className="flex h-full flex-col gap-0.5 overflow-y-auto p-3">
          ...
          <Link
            ...
            className={clsx(
              ...
              active
                ...
                : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-600 dark:text-slate-200 dark:hover:bg-slate-100 dark:hover:bg-slate-800/60 dark:hover:text-slate-800 dark:text-slate-200',
            )}
          >
            {m.icon} {m.label}
          </Link>
      ...
      <div className="mt-auto border-t border-slate-100 pt-3 dark:border-slate-200 dark:border-slate-800">
        <Link href="/" className="flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm text-slate-500 hover:bg-slate-100 dark:text-slate-600 dark:text-slate-200 dark:hover:bg-slate-100 dark:hover:bg-slate-800/60 dark:hover:text-slate-800 dark:text-slate-200">
    ...
    <div className="flex min-h-screen bg-slate-50 text-slate-800 dark:bg-[#0a0d12] dark:text-slate-800 dark:text-slate-200">
      ...
      {open && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="absolute start-0 top-0 h-full w-64 bg-white shadow-2xl dark:bg-[#121518]">
            <button onClick={() => setOpen(false)} className="absolute end-3 top-3 text-slate-600 dark:text-slate-200 hover:text-slate-700 dark:hover:text-slate-800 dark:text-slate-200"><X className="h-5 w-5" /></button>
            {nav}
          </aside>
        </div>
      )}
      ...
      <div className="min-w-0 flex-1">
        <header className="sticky top-0 z-40 flex items-center gap-3 border-b border-slate-200 bg-white/90 px-4 py-3 backdrop-blur lg:hidden dark:border-slate-200 dark:border-slate-800 dark:bg-[#121518]/90">
          ...
          <span className="ms-auto text-xs text-slate-500 dark:text-slate-600 dark:text-slate-200">{user?.fullName}</span>
        </header>
        <header className="sticky top-0 z-40 hidden items-center justify-end gap-3 border-b border-slate-200 bg-white/90 px-6 py-3 backdrop-blur lg:flex dark:border-slate-200 dark:border-slate-800 dark:bg-[#121518]/90">
          <ThemeToggle />
          <span className="text-sm text-slate-600 dark:text-slate-700 dark:text-slate-200">{user?.fullName}</span>
          <button
            onClick={() => { clearAuth(); router.push('/'); }}
            className="rounded-xl border border-slate-200 px-3 py-1.5 text-xs text-slate-500 transition hover:border-rose-200 hover:text-rose-600 dark:border-slate-200 dark:border-slate-800 dark:text-slate-600 dark:text-slate-200 dark:hover:border-rose-500/40 dark:hover:text-rose-300"
          >
            خروج
          </button>
        </header>
