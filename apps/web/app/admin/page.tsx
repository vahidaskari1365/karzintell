    ...
    <div>
          ...
          const inner = (
            <Card className="flex items-center gap-3 p-4">
              ...
              <div className="min-w-0">
                ...
                <p className="text-2xs text-slate-600 dark:text-slate-200">{k.sub}</p>
              </div>
            </Card>
          );
        ...
        {[
          ...
          <Link key={w.label} href={w.href}>
            <Card className={`flex items-center justify-between p-3.5 transition hover:border-orange-300 ${w.count > 0 ? 'border-orange-200 bg-orange-50/50' : ''}`}>
              <span className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-200">{w.icon}{w.label}</span>
              <span className={`text-base font-black ${w.count > 0 ? 'text-orange-600' : 'text-slate-700 dark:text-slate-200'}`}>{faNumber(w.count)}</span>
            </Card>
          </Link>
        ))}
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-5">
        ...
        <Card className="p-5 lg:col-span-3">
          ...
          {data.chart.length === 0 ? (
            <p className="py-10 text-center text-xs text-slate-600 dark:text-slate-200">هنوز فروشی ثبت نشده است</p>
          ) : (
        ...
        <Card className="p-0 lg:col-span-2">
          <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 px-5 py-4">
            <p className="text-sm font-bold text-slate-800 dark:text-slate-200">آخرین سفارش‌ها</p>
              ...
              <li key={o.id}>
                <Link href={`/admin/orders/${o.id}`} className="flex items-center justify-between px-5 py-3 hover:bg-white dark:bg-slate-900/40">
                  <div>
                    ...
                    <p className="text-2xs text-slate-600 dark:text-slate-200">{o.customerName}</p>
                  </div>
