        ...
        <>
          ...
          <Card className="mb-5 p-5">
            ...
            {data.series.length === 0 ? (
              <p className="py-8 text-center text-xs text-slate-600 dark:text-slate-300">در این بازه فروشی ثبت نشده</p>
            ) : (
              <div className="flex h-48 items-end gap-1 overflow-x-auto pb-6">
                {data.series.map((s) => (
                  <div key={s.period} className="group relative min-w-8 flex-1">
                    ...
                    />
                    <p className="absolute -bottom-5 left-1/2 -translate-x-1/2 whitespace-nowrap text-2xs text-slate-600 dark:text-slate-300" dir="ltr">{s.period.slice(5)}</p>
          ...
          <Card className="mb-5 p-5">
            ...
            {(profit || []).length === 0 ? (
              <p className="py-4 text-center text-xs text-slate-600 dark:text-slate-300">داده‌ای در این بازه نیست</p>
            ) : (
              <div className="max-h-56 space-y-1 overflow-y-auto">
                {(profit || []).map((p) => (
                  <div key={p.bucket} className="flex items-center justify-between rounded-xl px-3 py-2 text-xs odd:bg-white dark:bg-slate-900/40" >
                    <span className="text-slate-600 dark:text-slate-300" dir="ltr">{p.bucket}</span>
                    <span className={p.profit >= 0 ? 'font-bold text-emerald-700' : 'font-bold text-rose-700'}>
                      {toToman(p.profit)} تومان <span className="font-normal text-slate-600 dark:text-slate-300">({faNumber(p.orders)} سفارش)</span>
                    </span>
                  </div>
                ))}
              </div>
            ...
            <tbody>
              {topItems.map((t: any, i: number) => (
                <tr key={t.productId || t.id || i} className={tableCls.row}>
                  <td className={tableCls.td}><span className="flex h-6 w-6 items-center justify-center rounded-lg bg-slate-800/40 text-2xs font-black text-slate-600 dark:text-slate-300">{faNumber(i + 1)}</span></td>
              ...
              {topItems.length === 0 && <tr><td className="p-6 text-center text-xs text-slate-600 dark:text-slate-300">داده‌ای نیست</td></tr>}
            </tbody>
          </table>
        </div>
            ...
            <tbody>
              {lowItems.slice(0, 10).map((l: any, i: number) => (
                <tr key={l.variantId || i} className={tableCls.row}>
                  <td className={tableCls.td}><span className="text-xs font-medium">{l.productName}</span><br /><span className="text-2xs text-slate-600 dark:text-slate-300" dir="ltr">{l.sku}</span></td>
              ...
              {lowItems.length === 0 && <tr><td className="p-6 text-center text-xs text-slate-600 dark:text-slate-300">همه اقلام شارژ هستند 🎉</td></tr>}
            </tbody>
          </table>
        </div>
            ...
            <tbody>
              {(topCustomers || []).map((c: any, i: number) => (
                <tr key={c.id} className={tableCls.row}>
                  ...
                  <td className={tableCls.td}><span className="text-xs font-medium">{c.fullName}</span><br /><span className="text-2xs text-slate-600 dark:text-slate-300" dir="ltr">{c.phone}</span></td>
                  <td className={tableCls.td}>{faNumber(c.ordersCount)} سفارش</td>
                  <td className={`${tableCls.td} font-bold`}>{toToman(c.totalSpent)} تومان</td>
                  <td className={tableCls.td}><span className="text-2xs text-slate-600 dark:text-slate-300">{c.lastOrderAt ? new Date(c.lastOrderAt).toLocaleDateString('fa-IR') : '—'}</span></td>
                </tr>
              ))}
              {(topCustomers || []).length === 0 && <tr><td colSpan={5} className="p-6 text-center text-xs text-slate-600 dark:text-slate-300">داده‌ای نیست</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
