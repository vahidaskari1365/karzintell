              ...
              <tr key={r.id} className={tableCls.row}>
                <td className={tableCls.td}>
                  <p className="flex items-center gap-2 font-bold">
                    ...
                    {r.title}
                    {r.isSystem && <span className="rounded bg-slate-800/40 px-1.5 py-0.5 text-2xs text-slate-600 dark:text-slate-300">سیستمی</span>}
                  </p>
                  {r.description && <p className="mt-0.5 text-2xs text-slate-600 dark:text-slate-300">{r.description}</p>}
                </td>
                <td className={tableCls.td}><code className="text-xs text-slate-600 dark:text-slate-300" dir="ltr">{r.name}</code></td>
                <td className={tableCls.td}>{faNumber(r.userCount)}</td>
                <td className={tableCls.td}>
                  {r.permissions === '*' ? (
                    <Pill status="rejected" label="همه دسترسی‌ها (کامل)" />
                  ) : (
                    <span className="text-xs text-slate-600 dark:text-slate-300">{faNumber(r.permissions.length)} مجوز</span>
                  )}
                </td>
                <td className={`${tableCls.td} text-left`}>
                  <div className="flex justify-end gap-1">
                    ...
                    {canDelete && !r.isSystem && (
                      <button onClick={() => setDeleting(r)} className="rounded-lg p-2 text-slate-600 dark:text-slate-300 hover:bg-rose-50 hover:text-rose-600">
            ...
            return (
              <Card key={g} className="p-3.5">
                ...
                <div className="space-y-1.5">
                  {perms.map((p) => (
                    <label key={p.key} className="flex cursor-pointer items-center gap-2 text-xs text-slate-600 dark:text-slate-300">
                      <input
                        type="checkbox"
