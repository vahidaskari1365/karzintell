              ...
              {g.items.map((item) => {
                ...
                return (
                  <div key={item.key} className="grid items-center gap-2 sm:grid-cols-[220px_1fr_auto]">
                    <div>
                      <p className="text-xs font-medium text-slate-700 dark:text-slate-200" dir="ltr">{item.key}</p>
                      <p className="text-2xs text-slate-600 dark:text-slate-200">{item.type}</p>
                    </div>
                    {isBool ? (
                    ...
                    <label className="flex items-center gap-1.5 text-2xs text-slate-600 dark:text-slate-200" title="قابل خواندن از API عمومی">
                      <input
                        type="checkbox"
