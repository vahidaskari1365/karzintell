  ...
  return (
    <div>
      ...
      <div className="space-y-5">
        {(zones || []).length === 0 && (
          <Card className="p-8 text-center text-sm text-slate-600 dark:text-slate-200">
            <Truck className="mx-auto mb-2 h-8 w-8 text-slate-700 dark:text-slate-200" />
          ...
          <Card key={z.id}>
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 dark:border-slate-800 pb-3">
              <div className="flex items-center gap-3">
                ...
                {!z.isActive && <span className="rounded-full bg-slate-800/40 px-2 py-0.5 text-[10px] text-slate-600 dark:text-slate-200">غیرفعال</span>}
            ...
            {z.methods.length === 0 && <p className="text-xs text-slate-600 dark:text-slate-200">هنوز روشی برای این منطقه تعریف نشده است.</p>}
            {z.methods.length > 0 && (
                      ...
                      <tr key={m.id} className={tableCls.row}>
                        ...
                        <td className={tableCls.td}>
                          {m.isActive
                            ...
                            : <span className="rounded-full bg-slate-800/40 px-2 py-0.5 text-[10px] text-slate-600 dark:text-slate-200">غیرفعال</span>}
                        </td>
                        <td className={tableCls.td}>
                          <div className="flex gap-1">
                            <button onClick={() => setMethodDialog({ zoneId: z.id, method: m })} className="rounded-lg p-1.5 text-slate-600 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800/40 hover:text-slate-700 dark:text-slate-200"><Pencil className="h-4 w-4" /></button>
                            <button onClick={() => setRemoveMethod(m)} className="rounded-lg p-1.5 text-slate-600 dark:text-slate-200 hover:bg-rose-50 hover:text-rose-600"><Trash2 className="h-4 w-4" /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
