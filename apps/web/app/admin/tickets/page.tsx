              ...
              <tbody>
                {items.map((t) => (
                  <tr key={t.id} className={tableCls.row}>
                    ...
                    <td className={tableCls.td}><span className="text-xs text-slate-600 dark:text-slate-300">{faDateTime(t.updatedAt)}</span></td>
    ...
    <Dialog open onClose={onClose} title={`#${t.id} — ${t.subject}`} size="lg">
      <div className="mb-3 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1">
          ...
          {t.status !== 'closed' ? (
          <Button size="sm" variant="ghost" className="text-slate-600 dark:text-slate-300" onClick={() => setStatus.mutate('closed')}>
      ...
      <div className="max-h-96 space-y-3 overflow-y-auto rounded-xl bg-white dark:bg-slate-900/40 p-3">
        {isLoading ? (
          ...
          <p className="p-6 text-center text-xs text-slate-600 dark:text-slate-300">پیامی نیست</p>
        ) : (
          messages.map((m: any) => {
            ...
            return (
              <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] rounded-2xl p-3 text-sm ${m.isInternal ? 'border border-dashed border-amber-300 bg-amber-50' : mine ? 'bg-slate-800 text-white' : 'bg-white dark:bg-slate-900/40 border border-slate-300 dark:border-slate-700'}`}>
                  ...
                  <p className={`mt-1.5 text-2xs ${mine && !m.isInternal ? 'text-slate-600 dark:text-slate-300' : 'text-slate-600 dark:text-slate-300'}`}>{faDateTime(m.createdAt)}</p>
                </div>
              </div>
            );
          })
