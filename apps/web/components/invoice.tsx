...
export function Invoice({ order, storeName = 'فروشگاه کارزینتل' }: { order: OrderDetailType; storeName?: string }) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="rounded-2xl border border-slate-300 bg-[#181c20] p-8 text-sm shadow-sm print:rounded-none print:border-0 print:p-0 print:shadow-none">
        {/* سربرگ */}
        <div className="flex items-start justify-between border-b-2 border-slate-800 pb-4">
          <div>
            <h1 className="text-xl font-black">{storeName}</h1>
            <p className="mt-1 text-xs text-slate-400">فروشگاه اینترنتی لوازم دیجیتال — karzintell.ir</p>
          </div>
          <div className="text-left text-xs leading-6">
            ...
            <div>
              ...
              <b className={paid ? 'text-emerald-300' : 'text-amber-300'}>{paid ? 'پرداخت‌شده' : 'پرداخت‌نشده'}</b>
            </div>
          </div>
        </div>

        {/* مشخصات خریدار */}
        <div className="mt-4 grid grid-cols-2 gap-4 rounded-xl bg-[#10130f] p-4 text-xs leading-6 print:bg-[#181c20] print:outline print:outline-1 print:outline-slate-300">
          <div>
            <div className="mb-1 font-bold text-slate-400">مشخصات خریدار</div>
            <div>نام: {addr.receiverName || '—'}</div>
            <div>موبایل: <span dir="ltr">{addr.receiverPhone || '—'}</span></div>
          </div>
          <div>
            <div className="mb-1 font-bold text-slate-400">آدرس تحویل</div>
        ...
        <div className="mt-6 flex justify-end">
          <div className="w-64 space-y-1.5 text-xs">
            <div className="flex justify-between"><span className="text-slate-400">جمع اقلام:</span><span>{toToman(order.subtotal)}</span></div>
            {order.discountTotal > 0 && (
              <div className="flex justify-between text-emerald-300">
                <span>تخفیف{order.couponCode ? ` (${order.couponCode})` : ''}:</span><span>{toToman(order.discountTotal)}-</span>
              </div>
            )}
            <div className="flex justify-between"><span className="text-slate-400">هزینه ارسال{order.shippingMethod ? ` (${order.shippingMethod})` : ''}:</span><span>{order.shippingCost ? toToman(order.shippingCost) : 'رایگان'}</span></div>
            <div className="flex justify-between"><span className="text-slate-400">مالیات بر ارزش افزوده (۹٪):</span><span>{toToman(order.taxTotal)}</span></div>
        ...
        {order.customerNote && (
          <div className="mt-6 rounded-xl border border-white/10 p-3 text-xs text-slate-400">
            <b>یادداشت مشتری:</b> {order.customerNote}
          </div>
        )}

        {/* امضا */}
        <div className="mt-10 flex justify-between text-center text-xs text-slate-400">
